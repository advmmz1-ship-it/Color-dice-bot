// Color Dice Discord Bot — 1v1 Challenge Mode
//
...
const COLORS = [
  { id: 'red', name: 'Red', hex: 0xe74c3c, emoji: '🟥' },
  { id: 'orange', name: 'Orange', hex: 0xe67e22, emoji: '🟧' },
  { id: 'yellow', name: 'Yellow', hex: 0xf1c40f, emoji: '🟨' },
  { id: 'green', name: 'Green', hex: 0x2ecc71, emoji: '🟩' },
  { id: 'blue', name: 'Blue', hex: 0x3498db, emoji: '🟦' },
  { id: 'purple', name: 'Purple', hex: 0x9b59b6, emoji: '🟪' },
];
...
async function runColorRound(message, challenger, opponent, roundNumber) {
  const pickEmbed = new EmbedBuilder()
    .setTitle(roundNumber === 1 ? '🎲 Color Dice Challenge — Accepted!' : '🎲 Push! Roll again')
  ...
  selectCollector.on('collect', async (selectInteraction) => {
    ...
    if (matches === 2 || matches === 3) {
      // Push — reroll, opponent picks again
      const pushEmbed = new EmbedBuilder()
        .setTitle('🎲 Color Dice — Push!')
        .setDescription(
          `${opponent} chose ${chosenColor.emoji} **${chosenColor.name}**\n\n` +
            `**Rolled:** ${diceLine}\n(${namesLine})\n\n` +
            `${chosenColor.emoji} landed ${matches} times — that's a push. Rolling again...`
        )
        .setColor(0xf1c40f);
    ...
    const resultEmbed = new EmbedBuilder()
      ...
      .setFooter({ text: `${loser.username} — better luck next time` });
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.isChatInputCommand() && interaction.commandName === 'roll') {
    ...
    buttonCollector.on('collect', async (btnInteraction) => {
      ...
      // Accepted — hand off to the color-pick/roll loop
